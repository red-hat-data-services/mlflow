...
jest.mock('@databricks/web-shared/model-trace-explorer', () => ({
  ...
  isV4TraceId: jest.fn(() => false),
}));

jest.mock('../../shared/web-shared/model-trace-explorer/api', () => ({ getExperimentTraceV3: jest.fn() }));

jest.mock('@databricks/web-shared/genai-traces-table', () => ({
  getSpanAttribute: jest.fn((attributes: Record<string, unknown> | undefined, key: string) => attributes?.[key]),
}));

jest.mock('../../shared/web-shared/genai-traces-table/utils/TraceUtils', () => ({
  TRACKING_STORE_SPANS_LOCATION: 'TRACKING_STORE',
